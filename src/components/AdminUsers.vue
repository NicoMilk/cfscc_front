      ...
      <div class="content h-100 overflow-auto bg-light px-4" >  
        <div v-for="(user, index) in usersStore" :key="index">
          ...
          <div class="accordion" id="accordion">
            <div class="card">
              <div class="card-header" :id="'headingOne'+user.user_id">
                <h2 class="mb-0">
                  <button class="btn btn-link collapsed" type="button" data-toggle="collapse" :data-target="'#collapse'+user.user_id" aria-expanded="true" :aria-controls="'collapse'+user.user_id">
                    {{user.firstname}} {{user.lastname}} {{user.role}}
                  </button>
                </h2>
              </div>

              <div :id="'collapse'+user.user_id" class="collapse" :aria-labelledby="'headingOne'+user.user_id" data-parent="#accordion">
                <div class="card-body">
                                        ...
                                        <!-- <button type="button" class="btn btn-danger" @click="destroyUser(user.user_id)">Supprimer</button> -->
                                        </div>
                </div>
              </div>
            </div>
                            ...
                            <div class="modal-dialog">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <label for="lastname">Nom de famille :</label>
                                  <input type="text" class="form-control" v-model="user.lastname" id="lastname"/><br>
                                  <label for="firstname">Prénom :</label>
                                  <input type="text" class="form-control" v-model="user.firstname" id="firstname"/><br>
                                  <label for="email">Email :</label>
                                  <input type="email" class="form-control" v-model="user.email" id="email"/><br>
                                  <label for="phone">Tel. :</label>
                                  <input type="text" class="form-control" v-model="user.phone" id="phone"/><br>
                                  <label for="role">Role :</label>
                                ...
                                <div class="modal-footer">
                                  <button type="button" class="btn btn-primary" @click="registerNewUser">Mettre à jour</button>
                                  <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button>
                                  <button type="button" class="btn btn-danger" data-dismiss="modal">Supprimer l'utilisateur</button>
                                </div>
                              </div>
                            </div>
                          </div> -->
